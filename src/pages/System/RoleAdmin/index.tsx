...
function RoleAdminContainer() {
    ...
    const getData = async (page: { pageNum: number; pageSize: number }) => {
        ...
        setLoading(true)
        try {
            const res: Res = await dispatch.sys.getRoles(
        ...
        },
        {
            ...
            render: (v: number, record: TableRecordData) => {
                ...
                controls.forEach((item, index) => {
                    if (index) {
                        result.push(
                            <Divider key={`line${index}`} type="vertical" />
                        )
                    }
                    result.push(item)
                })
    ...
    const tableData = useMemo(() => {
        const resList = data.map((item, index): TableRecordData => {
            return {
                key: index,
                id: item.id,
        ...
        console.log('页面table列表是:', resList)
        return resList
    }, [page, data])
